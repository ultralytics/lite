...
export const FIXED_SHORTCUTS: { title: string; rows: { label: string; keys: string[] }[] }[] = [
  {
    ...
    rows: [
      ...
      { label: "Cut, copy, paste, or select all", keys: ["Mod+X", "Mod+C", "Mod+V", "Mod+A"] },
      { label: "Undo or redo", keys: ["Mod+Z", IS_MAC ? "Mod+Shift+Z" : "Mod+Y"] },
  ...
  },
  {
    ...
    rows: [
      ...
      { label: "Go to the next or previous match", keys: ["Mod+G", "Mod+Shift+G"] },
      { label: "Add the next occurrence to the selection", keys: ["Mod+D"] },
      { label: "Select every occurrence", keys: ["Mod+Shift+L"] },
      { label: "Add a cursor above or below", keys: ["Mod+Alt+ArrowUp", "Mod+Alt+ArrowDown"] },
      {
        label: "Move or copy the line up or down",
        keys: ["Alt+ArrowUp", "Alt+ArrowDown", "Alt+Shift+ArrowUp", "Alt+Shift+ArrowDown"],
      },
      { label: "Insert a blank line", keys: ["Mod+Enter"] },
      { label: "Select the parent syntax", keys: ["Mod+I"] },
      { label: "Toggle a comment", keys: ["Mod+/"] },
      { label: "Indent or outdent", keys: ["Tab", "Shift+Tab"] },
      { label: "Indent or outdent the selection", keys: ["Mod+[", "Mod+]", "Mod+Alt+\\"] },
      { label: "Delete the line", keys: ["Mod+Shift+K"] },
      { label: "Jump to the matching bracket", keys: ["Mod+Shift+\\"] },
      { label: "Toggle a block comment", keys: ["Alt+Shift+A"] },
      { label: "Undo or redo a selection", keys: ["Mod+U", IS_MAC ? "Mod+Shift+U" : "Alt+U"] },
      {
        label: "Fold or unfold",
        keys: IS_MAC ? ["Mod+Alt+[", "Mod+Alt+]"] : ["Mod+Shift+[", "Mod+Shift+]"],
      },
    ],
  },
];

export function fixedShortcut(combo: string) {
  return FIXED_SHORTCUTS.flatMap(({ rows }) => rows).find(({ keys }) =>
